...
- **DEBT-01** — Hostname-based ingress implemented as unplanned prerequisite work before Phase 3
  ...
  - /etc/hosts one-time manual step: `10.107.140.105 argo.local prom.local graf.local alman.local s3.local`
...
- **BUG-02** — /etc/hosts must use ingress-nginx ClusterIP, not 127.0.0.1, with qemu2 driver
  - Symptom: `curl: (7) Failed to connect to argo.local port 443` despite tunnel running.
